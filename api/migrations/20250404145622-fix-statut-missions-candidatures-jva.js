...
const query = {
  ...
  ops: { $elemMatch: { path: "/status", value: "CANCEL", originalValue: "VALIDATED" } },
...
module.exports = {
  async up(db, client) {
    ...
    const missionPatches = await db.collection("mission_patches").find(query).toArray();
    ...
    const updateMissionsResult = await MissionModel.updateMany({ _id: { $in: missionIds } }, { status: MISSION_STATUS.WAITING_VALIDATION }, { fromUser });
    console.log("🚀 ~ updateMissionsResult:", updateMissionsResult);

    const applicationPatches = await db.collection("application_patches").find(query).toArray();
    ...
    const updateApplicationsResult = await ApplicationModel.updateMany({ _id: { $in: applicationIds } }, { status: APPLICATION_STATUS.WAITING_VALIDATION }, { fromUser });
