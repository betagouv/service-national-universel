...
async function createLog(patch, actualYoung, event, value) {
  ...
  const response = await fetch(`${API_ANALYTICS_ENDPOINT}/log/young`, {
    ...
    body: JSON.stringify({
      ...
      user_source: young?.source || actualYoung?.source || "",
      user_classeId: young?.classeId || actualYoung?.classeId || "",
      user_etablissementId: young?.etablissementId || actualYoung?.etablissementId || "",
