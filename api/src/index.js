...
if (cluster.isMaster) {
  const workerCount = process.env.NODE_ENV === "production" ? os.cpus().length : 1;
  ...
  cluster.on("exit", (worker, code, signal) => {
    console.error(`Worker ${worker.id} died : ${code} ${signal}`);
    cluster.fork();
  });
} else {
  ...
  app.get("/", async (req, res) => {
    ...
    const d = new Date();
    res.status(200).send(`Hello World ! at ${d.toLocaleString()} from WORKER ${cluster.worker.id} and INSTANCE ${process.env.INSTANCE_NUMBER}`);
  });
