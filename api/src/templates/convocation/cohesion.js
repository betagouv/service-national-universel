...
async function fetchDataForYoung(young) {
    ...
    if (!young.sessionPhase1Id || (!isLocalTransport(young) && !young.meetingPointId && young.deplacementPhase1Autonomous !== "true" && young.source !== "CLE")) throw `young ${young.id} unauthorized`;
...
function render(doc, { young, session, cohort, center, service, meetingPoint, ligneBus, ligneToPoint }) {
    ...
    doc.lineWidth(1).moveTo(MARGIN, doc.y - 2 ).lineTo(page.width - MARGIN, doc.y - 2).stroke();
    doc.font(FONT)
    doc.fontSize(8).text(`Attention à bien respecter l'horaire indiqué. En cas de retard, vous ne pourrez pas effectuer votre séjour de cohésion. Votre représentant légal doit rester jusqu'à votre prise en charge par les accompagnateurs.`, MARGIN + 20, undefined, {width: page.width - 2 * (MARGIN + 20)});
    doc.moveTo(MARGIN, doc.y + 5).lineTo(page.width - MARGIN, doc.y + 5).stroke();
    ...
    doc.list(
        ...
        doc.x + LIST_INDENT,
        undefined,
        { bulletRadius: 1.5, textIndent: LIST_INDENT, baseline: -7 },
    );
    ...
    doc.font(FONT_BOLD)
    doc.text(`Votre représentant légal veillera à bien respecter ces modalités de retour (horaire, lieu de prise en charge). Vous ne pourrez repartir seul, sauf si vous présentez une autorisation de votre représentant légal.`)

    doc.moveDown(0.5);

    doc.font(FONT)
    doc.text(`Afin que votre séjour se déroule dans les meilleures conditions, nous vous rappelons que chaque volontaire, lors de son inscription s'est engagé à respecter le `, {continued: true})
...
async function generateCohesion(outStream, young) {
  ...
  module.exports = { generateCohesion };
