...
const getUserAttributes = async (user) => {
  ...
  const profilLink = isYoung(user) ? `${config.ADMIN_URL}/volontaire/${user._id}` : `${config.ADMIN_URL}/user/${user._id}`;
  ...
  if (isYoung(user)) {
    ...
    if (departmentReferentPhase2) departmentReferentPhase2Link = `${config.ADMIN_URL}/user/${departmentReferentPhase2._id}`;
    ...
    userAttributes.push({ name: "lien vers candidatures", value: `${config.ADMIN_URL}/volontaire/${user._id}/phase2` });
    userAttributes.push({
      name: "lien vers équipe départementale",
      value: `${config.ADMIN_URL}/user?department=${user.department}&role=referent_department`,
    });
    ...
    if (user.role === ROLES.REFERENT_DEPARTMENT || user.role === ROLES.REFERENT_REGION) {
      userAttributes.push({
        name: "lien vers équipe départementale",
        value: `${config.ADMIN_URL}/user?department=${user.department.join("~")}&role=referent_department`,
      });
    ...
    if (user.role === ROLES.REFERENT_REGION) {
      userAttributes.push({
        ...
        value: `${config.ADMIN_URL}/user?region=${user.region}&role=referent_region`,
      });
    }
  }
