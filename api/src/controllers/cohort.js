...
router.put("/:cohort", passport.authenticate([ROLES.ADMIN], { session: false }), async (req, res) => {
  try {
    ...
    const { error: bodyError, value: body } = Joi.object({
      ...
      validationDateForTerminaleGrade: Joi.date().required(),
      ...
      sessionEditionOpenForReferentRegion: Joi.boolean().required(),
      sessionEditionOpenForReferentDepartment: Joi.boolean().required(),
    ...
    cohort.set({
      ...
      sessionEditionOpenForReferentRegion: body.sessionEditionOpenForReferentRegion,
      sessionEditionOpenForReferentDepartment: body.sessionEditionOpenForReferentDepartment,
