...
router.put("/relance", passport.authenticate("young", { session: false, failWithError: true }), async (req, res) => {
  try {
    ...
    // Si au moins une des pieces d'identités est valide au moment du séjour, on n'envoie pas.
    // (dans le cas où un volontaire ajouterait une piece valide apres en avoir ajouté une trop vieille et que ces parents aient reçu une notif).
