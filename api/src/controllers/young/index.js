...
router.post("/signup", async (req, res) => {
  try {
    ...
    const userExist = await YoungObject.find({ lastName, firstName, birthdateAt });
    if (userExist.length > 0) return res.status(409).send({ ok: false, code: ERRORS.USER_ALREADY_REGISTERED });
