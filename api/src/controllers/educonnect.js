...
router.post("/callback", passport.authenticate("educonnect"), async (req, res) => {
  try {
    ...
    const { error, value } = Joi.object({
      ...
      FrEduCtEleveINEHash: Joi.string().trim().length(32),
      FrEduCtEleveUAI: Joi.string().trim(),
      FrEduCtDateNaissance: Joi.string().trim(),
      FrEduCtEleveNiveau: Joi.string().trim(),
      FrEduUrlRetour: Joi.string().trim(),
    })
      .unknown()
...
router.get("/test-output", async (req, res) => {
  try {
    const attributes = {
      ...
      "urn:oid:1.3.6.1.4.1.20326.10.999.1.67": "2005-11-05",
      "urn:oid:2.5.4.4": "LECOMTE02545",
      "urn:oid:1.3.6.1.4.1.20326.10.999.1.64": "0278dd38a2de195c7f0598fca3590320",
