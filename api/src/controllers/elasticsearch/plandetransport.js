...
router.post("/:action(search|export)", passport.authenticate(["referent"], { session: false, failWithError: true }), async (req, res) => {
  try {
    ...
    const { queryFilters, page, exportFields, sort, error, size } = joiElasticSearch({ filterFields, sortFields, body: req.body });
    ...
    if (req.params.action === "export") {
      const response = await allRecords("plandetransport", hitsRequestBody.query, exportFields);
