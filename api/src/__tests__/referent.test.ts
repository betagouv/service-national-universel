...
describe("Referent", () => {
  ...
  describe("PUT /referent/young/:id", () => {
    ...
    it("should not update young if department goal reached", async () => {
      ...
      let completionObjectif = await getCompletionObjectifStats(inscriptionGoal.department!, testName);
      ...
      response = (
        await createYoungThenUpdate(
          ...
          { region: inscriptionGoal.region, department: inscriptionGoal.department, cohortId: cohort._id },
        )
      ).response;
      expect(response.statusCode).not.toEqual(200);
      expect(response.body.code).toBe(FUNCTIONAL_ERRORS.INSCRIPTION_GOAL_NOT_DEFINED);
    ...
    it("should not update young if region goal reached (not department)", async () => {
      ...
      let completionObjectif = await getCompletionObjectifStats(inscriptionGoal.department!, cohort.name);
