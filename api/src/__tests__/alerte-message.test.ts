...
beforeAll(async () => {
  ...
  await addPermissionHelper([ROLES.ADMIN, ROLES.REFERENT_DEPARTMENT], PERMISSION_RESOURCES.ALERTE_MESSAGE, PERMISSION_ACTIONS.FULL);
});
afterAll(dbClose);
...
describe("AlerteMessage Routes", () => {
  ...
  describe("GET /all", () => {
    it("should return 200 and all messages for authorized users", async () => {
      ...
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).get("/alerte-message/all");
      expect(response.statusCode).toBe(200);
      expect(response.body.ok).toBe(true);
      expect(response.body.data.length).toEqual(1);
    });
    it("should return 403 for unauthorized users", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.VISITOR })).get(`/alerte-message/all`);
  ...
  describe("GET /:id", () => {
    it("should return 200 and the message for valid ID and authorized users", async () => {
      ...
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).get(`/alerte-message/${message._id}`);
      expect(response.statusCode).toBe(200);
      expect(response.body.ok).toBe(true);
    ...
    it("should return 403 for valid ID and unauthorized users", async () => {
      const message = await AlerteMessageModel.create(getNewAlerteMessageFixture());
      const response = await request(await getAppHelperWithAcl({ role: ROLES.VISITOR })).get(`/alerte-message/${message._id}`);
      expect(response.statusCode).toBe(403);
      expect(response.body.ok).toBe(false);
    });
    it("should return 400 for invalid ID format", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).get("/alerte-message/invalid_id");
      expect(response.statusCode).toBe(400);
      expect(response.body.ok).toBe(false);
    });
    it("should return 404 for non-existing message ID", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).get("/alerte-message/123456789012345678901234");
  ...
  describe("GET /", () => {
    it("should return 200 and messages for the user's role", async () => {
      ...
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).get("/alerte-message");
      expect(response.statusCode).toBe(200);
      expect(response.body.ok).toBe(true);
      expect(response.body.data.length).toEqual(1);
    });
    it("should return 403 for an unauthorized users", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.VISITOR })).get("/alerte-message");
      expect(response.statusCode).toBe(403);
      expect(response.body.ok).toBe(false);
    });
    it("should return 200 and empty messages when no messages for this role", async () => {
      await AlerteMessageModel.create(getNewAlerteMessageFixture({ to_role: ["admin"] }));
      await AlerteMessageModel.create(getNewAlerteMessageFixture({ to_role: ["referent_region"] }));
      const response = await request(await getAppHelperWithAcl({ role: ROLES.REFERENT_DEPARTMENT })).get("/alerte-message");
      expect(response.statusCode).toBe(200);
      expect(response.body.ok).toBe(true);
  ...
  describe("DELETE /:id", () => {
    it("should delete a message and return 200 for authorized users", async () => {
      const message = await AlerteMessageModel.create(getNewAlerteMessageFixture());
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).delete(`/alerte-message/${message._id}`);
      expect(response.statusCode).toBe(200);
      expect(response.body.ok).toBe(true);
    });
    it("should return 403 for valid id and unauthorized users", async () => {
      const message = await AlerteMessageModel.create(getNewAlerteMessageFixture());
      const response = await request(await getAppHelperWithAcl({ role: ROLES.VISITOR })).delete(`/alerte-message/${message._id}`);
      expect(response.statusCode).toBe(403);
      expect(response.body.ok).toBe(false);
    });
    it("should return 400 for invalid ID format", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).delete("/alerte-message/invalid_id");
      expect(response.statusCode).toBe(400);
      expect(response.body.ok).toBe(false);
    });
    it("should return 404 for non-existing message ID", async () => {
      const response = await request(await getAppHelperWithAcl({ role: ROLES.ADMIN })).delete("/alerte-message/123456789012345678901234");
      expect(response.statusCode).toBe(404);
      expect(response.body.ok).toBe(false);
    });
  });
