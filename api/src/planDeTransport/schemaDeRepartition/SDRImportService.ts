...
export const importSchemaDeRepartition = async (pdrFilePath: string) => {
  // const timestamp = `${new Date().toISOString()?.replaceAll(":", "-")?.replace(".", "-")}`;

  // const data = fs.readFileSync(filePath);

  // const csvBuffer = XLSXToCSVBuffer(filePath);
  // const parsedContent = await readCSVBuffer(csvBuffer);
  ...
  for (const pdr of filteredPdrs) {
    ...
    try {
      if (!pdr.matricule) {
        logger.warn(`importSchemaDeRepartition() - Pdr ${pdr.name} has no matricule, skipping`);
        report.push({
          name: pdr.name,
          action: "nothing",
        ...
        logger.warn(`importSchemaDeRepartition() - Pdr ${pdr.matricule} has no name, skipping`);
        report.push({
          name: pdr.name,
          action: "nothing",
...
const getPdrExtraInfos = async (pdr: PointDeRassemblementImportMapped, foundPdr?: PointDeRassemblementDocument) => {
  ...
  if (cohorts.length !== 4) {
    throw Error("importSchemaDeRepartition() - Cohortes CLE 05 et CLE 06 non trouvées");
  }
  ...
  try {
    ...
    logger.warn(`importSchemaDeRepartition() - geocoding pdr ${pdr.matricule}: ${e.message}`);
  ...
  if (!location?.lat || !location.lon) {
    throw Error(`importSchemaDeRepartition() - No suitable location found`);
  }
