...
export default function DropdownLastActivityDate({ name, selectedLastActivityDate, setSelectedLastActivityDate }) {
  const onChange = (lastActivityDate) => {
    setSelectedLastActivityDate(lastActivityDate);
  };
  const defaultMonth = selectedLastActivityDate?.from ? toUtcLocally(selectedLastActivityDate?.from) : today();
  const selected = {
    from: selectedLastActivityDate?.from ? toUtcLocally(selectedLastActivityDate?.from) : undefined,
    to: selectedLastActivityDate?.to ? toUtcLocally(selectedLastActivityDate?.to) : undefined,
  };
  ...
  return (
    <div>
      ...
      <div className="mt-2 grid grid-cols-1 gap-1">
        {selectedLastActivityDate?.from && <span className="rounded-xl bg-purple-100 px-1 text-center font-medium text-purple-800">Date sélectionnée</span>}
      </div>
    </div>
  );
}
