    ...
    <Router history={history}>
      ...
      <div className="main">
        ...
        {environment === "production" ? (
          <Footer />
        ) : ["preinscription", "auth", "inscription2023", "reinscription", "representants-legaux"].findIndex((route) => location.pathname.includes(route)) === -1 ? (
          <Footer />
        ) : null}
      </div>
    </Router>
...
const Espace = () => {
  ...
  if (!young) {
    ...
    return environment === "production" ? (
      <Redirect to={{ search: redirect && redirect !== "logout" ? `?redirect=${redirect}` : "", pathname: "/inscription" }} />
    ) : (
