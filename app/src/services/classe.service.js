...
export function formatClass(data) {
  ...
  const isInscriptionOpen = [STATUS_CLASSE.INSCRIPTION_IN_PROGRESS, STATUS_CLASSE.CREATED].includes(status) && !isFull && new Date().valueOf() <= new Date("2024-02-08 23:59:59").valueOf();
  const classe = {
    id: data._id,
    name,
