...
export default function SignupButtonContainer({ onClickNext, onClickPrevious, labelNext = "Continuer", labelPrevious = "Précédent", disabled = false, collapsePrevious = false }) {
  return (
    <div className="w-full mx-auto fixed md:relative bottom-0 left-0 bg-white shadow-ninaInverted md:shadow-none py-4 px-[1rem] md:px-0 ">
      ...
      <div className={`flex ${!collapsePrevious && "flex-col-reverse md:flex-row"} justify-center md:justify-end gap-2`}>
        {onClickPrevious && (
          <button
            className={`${
            ...
            {onClickNext && collapsePrevious ? <FiChevronLeft className="block md:hidden" /> : <p className="w-64 block md:hidden">{labelPrevious}</p>}
