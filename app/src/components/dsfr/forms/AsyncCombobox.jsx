...
export default function AsyncCombobox({ label, hint = "", getOptions, updateData, error }) {
  ...
  const handleChangeQuery = async (e) => {
    ...
    const [options, error] = await getOptions(e.target.value);
      ...
      <div className="relative">
        {query?.trim().length > 2 && (
          <div className="bg-white border flex flex-col absolute z-10 -top-1 w-full shadow">
            {options.length ? <Dropdown options={options} handleSelect={handleSelect} /> : <p className="p-3 text-gray-800 text-center">{hint}</p>}
          </div>
        )}
      </div>
...
function Dropdown({ options, handleSelect }) {
  return options.map((option) => (
    <button key={option} onClick={() => handleSelect(option)} className="p-2 hover:bg-blue-france-sun-113 hover:text-white w-full flex justify-between">
