  ...
  const handleMail = async () => {
    try {
      const { ok } = await api.post(`/young/${young._id}/email/${SENDINBLUE_TEMPLATES.young.LINK}`, {
        ...
        link: CDN_BASE_URL + "/file/fiche-sanitaire.pdf" + "?utm_campaign=transactionnel+telecharger+docum&utm_source=notifauto&utm_medium=mail+410+telecharger",
      });
        ...
        {enabled ? (
          <>
            <div className="flex flex-row items-center justify-center pb-4 lg:!pb-0">
              ...
              <a target="blank" href={CDN_BASE_URL + "/file/fiche-sanitaire.pdf"} onClick={handleDownload}>
                <button
                  type="button"
