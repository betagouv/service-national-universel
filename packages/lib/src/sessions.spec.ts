...
describe("sessions", () => {
  it("should return valid formatted period for same month and year", () => {
    let formattedPeriod = getCohortPeriod({
      name: "MyCohort",
      dateStart: new Date("2024-06-03T00:00:00.000+00:00"),
      dateEnd: new Date("2024-06-14T00:00:00.000+00:00"),
    });
  ...
  it("should return valid formatted period for different months in the same year", () => {
    let formattedPeriod = getCohortPeriod({
      name: "MyCohort",
      dateStart: new Date("2024-05-26T00:00:00.000+00:00"),
      dateEnd: new Date("2024-06-07T00:00:00.000+00:00"),
    });
  ...
  it("should return valid formatted period for different years", () => {
    let formattedPeriod = getCohortPeriod({
      name: "MyCohort",
      dateStart: new Date("2024-12-28T00:00:00.000+00:00"),
      dateEnd: new Date("2025-01-10T00:00:00.000+00:00"),
    });
  ...
  it("should return cohort name if dates are missing", () => {
    let formattedPeriod = getCohortPeriod({ name: "MyCohort" } as any);
    expect(formattedPeriod).toBe("MyCohort");

    formattedPeriod = getCohortPeriod({ name: "MyCohort", dateStart: null, dateEnd: null } as any);
    expect(formattedPeriod).toBe("MyCohort");
  });

  it("should return 'à venir' if cohort name is 'à venir'", () => {
    const formattedPeriod = getCohortPeriod({ name: "à venir" } as any);
