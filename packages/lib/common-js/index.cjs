...
var regionList = ["Auvergne-Rhône-Alpes", "Bourgogne-Franche-Comté", "Bretagne", "Centre-Val de Loire", "Corse", "Grand Est", "Hauts-de-France", "Île-de-France", "Normandie", "Nouvelle-Aquitaine", "Occitanie", "Pays de la Loire", "Provence-Alpes-Côte d'Azur", "Guadeloupe", "Martinique", "Guyane", "La Réunion", "Saint-Pierre-et-Miquelon", "Mayotte", "Terres australes et antarctiques françaises", "Wallis-et-Futuna", "Polynésie française", "Nouvelle-Calédonie"];
...
var getRegionForEligibility = function getRegionForEligibility(young) {
  ...
  if (!region) {
    var dep = (young === null || young === void 0 ? void 0 : young.schoolDepartment) || (young === null || young === void 0 ? void 0 : young.department) || getDepartmentByZip(young === null || young === void 0 ? void 0 : young.zip);
    if (dep && (!isNaN(dep) || ["2A", "2B", "02A", "02B"].includes(dep))) {
      if (dep.substring(0, 1) === "0" && dep.length === 3) dep = departmentLookUp[dep.substring(1)];else dep = departmentLookUp[dep];
    }
...
var sameGeography = function sameGeography(actor, target) {
  var actorAndTargetInTheSameRegion = (actor === null || actor === void 0 ? void 0 : actor.region) && (actor === null || actor === void 0 ? void 0 : actor.region) === (target === null || target === void 0 ? void 0 : target.region) || region2department[actor === null || actor === void 0 ? void 0 : actor.region].includes(target === null || target === void 0 ? void 0 : target.department);
  var actorAndTargetInTheSameDepartment = (actor === null || actor === void 0 ? void 0 : actor.department) && (actor === null || actor === void 0 ? void 0 : actor.department.includes(target === null || target === void 0 ? void 0 : target.department));
...
function canDeleteReferent(_ref) {
  ...
  var authorized = isAdmin || referentRegionFromTheSameRegion || referentDepartmentFromTheSameRegion || referentResponsibleFromTheSameRegion || responsibleFromTheSameRegion || visitorFromTheSameRegion || headCenterFromTheSameRegion || referentDepartmentFromTheSameDepartment || responsibleFromTheSameDepartment || headCenterFromTheSameDepartment || responsibleFromSameStructure && !isMe || supervisorOfMainStructure;
  return authorized;
}
...
function canUpdateReferent(_ref2) {
  ...
  var isMe = actor.id === originalTarget.id;
  var isAdmin = actor.role === ROLES.ADMIN;
  ...
  [ROLES.RESPONSIBLE, ROLES.SUPERVISOR].includes(originalTarget.role) && withoutChangingRole;
  var isSupervisorModifyingTeamMember =
  // Is supervisor...
  actor.role === ROLES.SUPERVISOR &&
  // ... modifying team member ...
  [ROLES.RESPONSIBLE, ROLES.SUPERVISOR].includes(originalTarget.role) && actor.structureId === originalTarget.structureId;
  ...
  var geographicTargetData = {
    ...
    department: (_originalTarget$depar = originalTarget.department) !== null && _originalTarget$depar !== void 0 && _originalTarget$depar.length && originalTarget.department.length !== 1 && originalTarget.department[0] !== "" ? originalTarget.department : [structure === null || structure === void 0 ? void 0 : structure.department]
  };
  ...
  var authorized = (isMeWithoutChangingRole || isAdmin || isSupervisorModifyingTeamMember || isResponsibleModifyingResponsibleWithoutChangingRole || isReferentModifyingReferentWithoutChangingRole || isReferentModifyingHeadCenterWithoutChangingRole) && (actor.role === ROLES.REFERENT_REGION ? isActorAndTargetInTheSameRegion || isReferentModifyingHeadCenterWithoutChangingRole : true) && (actor.role === ROLES.REFERENT_DEPARTMENT ? isActorAndTargetInTheSameDepartment || isReferentModifyingHeadCenterWithoutChangingRole : true);
  return authorized;
}
...
function canViewYoungFile(actor, target) {
  ...
  var authorized = isAdmin || isReferentDepartmentFromTargetDepartment || isReferentRegionFromTargetRegion || isReferentCenterFromSameDepartmentTargetCenter || isReferentCenterFromSameRegionTargetCenter;
  return authorized;
}
...
function canCreateOrUpdateProgram(user, program) {
  ...
  return isAdminOrReferent && !(user.role === ROLES.REFERENT_DEPARTMENT && !user.department.includes(program.department) || user.role === ROLES.REFERENT_REGION && user.region !== program.region);
}
...
var isTemporaryAffected = function isTemporaryAffected(young) {
  return (young === null || young === void 0 ? void 0 : young.statusPhase1) === "WAITING_AFFECTATION" && ["AFFECTED", "WAITING_LIST"].includes(young === null || young === void 0 ? void 0 : young.statusPhase1Tmp);
};
...
function canAssignManually(actor, young, cohort) {
  ...
  return actor.role === ROLES.ADMIN && cohort.manualAffectionOpenForAdmin || actor.role === ROLES.REFERENT_REGION && actor.region === young.region && cohort.manualAffectionOpenForReferentRegion || actor.role === ROLES.REFERENT_DEPARTMENT && actor.department.includes(young.department) && cohort.manualAffectionOpenForReferentDepartment;
}
...
function canUpdateLigneBus(actor) {
  return [ROLES.ADMIN, ROLES.TRANSPORTER
...
var SENDINBLUE_TEMPLATES = {
  FORGOT_PASSWORD: "157",
  invitationReferent: (_invitationReferent = {}, _defineProperty(_invitationReferent, ROLES.REFERENT_DEPARTMENT, "158"), _defineProperty(_invitationReferent, ROLES.REFERENT_REGION, "159"), _defineProperty(_invitationReferent, ROLES.RESPONSIBLE, "160"), _defineProperty(_invitationReferent, ROLES.SUPERVISOR, "160"), _defineProperty(_invitationReferent, ROLES.ADMIN, "161"), _defineProperty(_invitationReferent, ROLES.HEAD_CENTER, "162"), _defineProperty(_invitationReferent, ROLES.VISITOR, "286"), _defineProperty(_invitationReferent, ROLES.DSNJ, "662"), _defineProperty(_invitationReferent, ROLES.TRANSPORTER, "662"), _defineProperty(_invitationReferent, "NEW_STRUCTURE", "160"), _defineProperty(_invitationReferent, "NEW_STRUCTURE_MEMBER", "163"), _invitationReferent),
...
var SENDINBLUE_SMS = {
  PARENT1_CONSENT: {
    template: function template(young, cta) {
      return "Bonjour,\nNous avons besoin de votre accord pour que ".concat(young.firstName, " ").concat(young.lastName, " participe au SNU !\nCliquez ici pour le donner : ").concat(cta, "\nService National Universel");
    },
    tag: "sms_1"
  },
  PARENT2_CONSENT: {
    template: function template(young, cta) {
      return "Bonjour,\n".concat(young.firstName, " ").concat(young.lastName, " souhaite participer au SNU !\nDonnez votre accord pour l\u2019utilisation de son droit \xE0 l\u2019image en cliquant ici : ").concat(cta, "\nService National Universel");
    },
    tag: "sms_2"
  },
  PARENT1_CONSENT_REMINDER: {
    template: function template(young, cta) {
      return "Bonjour,\nVous n\u2019avez pas encore donn\xE9 votre accord pour que ".concat(young.firstName, " ").concat(young.lastName, " participe au SNU !\nCliquez ici pour le donner : ").concat(cta, "\nService National Universel");
    },
    tag: "sms_3"
  },
  OUTDATED_ID_PROOF: {
    template: function template(young, cta) {
      return "Bonjour,\nLa pi\xE8ce d\u2019identit\xE9 de ".concat(young.firstName, " ").concat(young.lastName, " est ou sera p\xE9rim\xE9e au moment du s\xE9jour.\nCliquez ici pour les d\xE9marches \xE0 suivre : ").concat(cta, "\nService National Universel");
    },
    tag: "sms_4"
  }
};
...
var CONSENTMENT_TEXTS = {
  young: ["A lu et accepte les Conditions générales d'utilisation de la plateforme du Service national universel ;", "A pris connaissance des modalités de traitement de mes données personnelles ;", "Est volontaire, sous le contrôle des représentants légaux, pour effectuer la session 2022 du Service National Universel qui comprend la participation au séjour de cohésion puis la réalisation d'une mission d'intérêt général ;", "Certifie l'exactitude des renseignements fournis ;", "Si en terminale, a bien pris connaissance que si je suis convoqué(e) pour les épreuves du second groupe du baccalauréat entre le 6 et le 8 juillet 2022, je ne pourrai pas participer au séjour de cohésion entre le 3 et le 15 juillet 2022(il n’y aura ni dérogation sur la date d’arrivée au séjour de cohésion ni report des épreuves)."],
  parents: ["Confirmation d'être titulaire de l'autorité parentale / le représentant légal du volontaire ;", "Autorisation du volontaire à participer à la session 2022 du Service National Universel qui comprend la participation au séjour de cohésion puis la réalisation d'une mission d & apos; intérêt général ;", "Engagement à renseigner le consentement relatif aux droits à l'image avant le début du séjour de cohésion ;", "Engagement à renseigner l'utilisation d'autotest COVID avant le début du séjour de cohésion ;", "Engagement à remettre sous pli confidentiel la fiche sanitaire ainsi que les documents médicaux et justificatifs nécessaires à son arrivée au centre de séjour de cohésion ;", "Engagement à ce que le volontaire soit à jour de ses vaccinations obligatoires, c'est-à-dire anti-diphtérie, tétanos et poliomyélite (DTP), et pour les volontaires résidents de Guyane, la fièvre jaune."]
};
...
var MINISTRES = [{
  date_end: "07-25-2020",
  ministres: ["Jean-Michel Blanquer, Ministre de l’Éducation Nationale, de la Jeunesse et des Sports", "Gabriel Attal, Secrétaire d’État auprès du ministre Éducation Nationale et de la Jeunesse"],
  template: "certificates/certificateTemplate-2019.png"
}, {
  date_end: "05-19-2022",
  ministres: ["Jean-Michel Blanquer, Ministre de l’Éducation Nationale, de la Jeunesse et des Sports", "Sarah El Haïry, Secrétaire d’État auprès du ministre de l'Éducation Nationale et de la Jeunesse"],
  ...
  date_end: "04-05-2023",
  ministres: ["Pap Ndiaye, Ministre de l’Éducation Nationale et de la Jeunesse", "Sébastien Lecornu, Ministre des Armées", "Sarah El Haïry, Secrétaire d'État après du ministre de 'Éducation nationale et de la Jeunesse et du ministre des Armées, \
...
var FILE_KEYS = ["cniFiles", "highSkilledActivityProofFiles", "parentConsentmentFiles", "autoTestPCRFiles", "imageRightFiles", "dataProcessingConsentmentFiles", "rulesFiles", "equivalenceFiles"];
...
var ENGAGEMENT_TYPES = ["Service Civique", "BAFA", "Jeune Sapeur Pompier", "Certification Union Nationale du Sport scolaire (UNSS)", "Engagements lycéens", "Préparation militaire hors offre MIG des armées"];
var UNSS_TYPE = ["Jeunes arbitres", "Jeunes juges", "Jeunes coachs / capitaines", "Jeunes dirigeants", "Jeunes écoresponsables", "Jeunes interprètes", "Jeunes organisateurs", "Jeunes reporters", "Jeunes secouristes", "Jeunes vice-présidents"];
var ENGAGEMENT_LYCEEN_TYPES = ["Elu au sein du conseil des délégués pour la vie lycéenne (CVL)", "Elu au sein du conseil académique de la vie lycéenne (CAVL)", "Elu au sein des conseils régionaux des jeunes"];
var GRADES = {
...
var translateSource = function translateSource(bool) {
  if (bool === "true") return "JVA";else return "SNU";
};
...
var ticketStateIdByName = function ticketStateIdByName(name) {
  return Number(Object.keys(ticketStates).reduce(function (ret, key) {
...
var youngExportFields = [{
  ...
  desc: ["Adresse postale", "Code postal", "Ville", "Pays", "Nom de l'hébergeur", "Prénom de l'hébergeur", "Lien avec l'hébergeur", "Adresse - étranger", "Code postal - étranger", "Ville - étranger", "Pays - étranger"],
  ...
  fields: ["qpv", "populationDensity", "handicap", "ppsBeneficiary", "paiBeneficiary", "specificAmenagment", "specificAmenagmentType", "reducedMobilityAccess", "handicapInSameDepartment", "allergies", "highSkilledActivity", "highSkilledActivityType", "highSkilledActivityInSameDepartment", "highSkilledActivityProofFiles", "medicosocialStructure", "medicosocialStructureName", "medicosocialStructureAddress", "medicosocialStructureZip", "medicosocialStructureCity"]
}, {
  id: "representative1",
  ...
  fields: ["parent1Status", "parent1FirstName", "parent1LastName", "parent1Email", "parent1Phone", "parent1Address", "parent1Zip", "parent1City", "parent1Department", "parent1Region", "parent1PhoneZone"]
}, {
  id: "representative2",
  title: "Représentant légal 2",
  desc: ["Statut", "Nom", "Prénom", "Email", "Téléphone", "Adresse", "Code postal", "Ville", "Département et région du représentant légal"],
  fields: ["parent2Status", "parent2FirstName", "parent2LastName", "parent2Email", "parent2Phone", "parent2Address", "parent2Zip", "parent2City", "parent2Department", "parent2Region", "parent2PhoneZone"]
}, {
  id: "consent",
  title: "Consentement",
  ...
  desc: ["Autonomie", "Numéro de bus", "Les informations de transport transmises par les services locaux", "ID du bus", "Nom du point de rassemblement", "Adresse du point de rassemblement", "Ville du point de rassemblement", "Dates d'aller et de retour", "Heures de départ et de convocation", "Heure de retour", "Voyage en avion"],
  ...
  fields: ["domains", "professionnalProject", "professionnalProjectPrecision", "period", "periodRanking", "mobilityNearSchool", "mobilityNearHome", "mobilityNearRelative", "mobilityNearRelativeName", "mobilityNearRelativeAddress", "mobilityNearRelativeZip", "mobilityNearRelativeCity", "mobilityTransport", "missionFormat", "engaged", "engagedDescription", "desiredLocation"]
}, {
  id: "accountDetails",
  title: "Compte",
...
var missionExportFields = [{
  ...
  desc: ["Titre de la mission", "Date du début", "Date de fin", "Nombre de volontaires recherchés", "Places restantes sur la mission", "Visibilité de la mission", "Source de la mission"],
...
var structureExportFields = [{
  ...
  id: "team",
  title: "Equipe",
  desc: ["Taille d'équipe", "Membre 1 - Nom", "Membre 1 - Prénom", "Membre 1 - Email", "Membre 2 - Nom", "Membre 2 - Prénom", "Membre 2 - Email", "Membre 3 - Nom", "Membre 3 - Prénom", "Membre 3 - Email"],
  fields: []
}, {
  id: "status",
  title: "Statut",
...
var applicationExportFieldsStructure = [{
  ...
  id: "application",
  title: "Candidature",
  desc: ["Statut de la candidature", "Choix - Ordre de la candidature", "Candidature créée le", "Candidature mise à jour le", "Statut du contrat d'engagement", "Pièces jointes à l’engagement", "Statut du dossier d'éligibilité PM"],
  fields: ["youngId", "status", "priority", "createdAt", "updatedAt", "contractAvenantFiles", "justificatifsFiles", "feedBackExperienceFiles", "othersFiles", "contractStatus", "eligibilityStatus"]
}, {
  id: "missionInfo",
  title: "Informations sur la mission",
  ...
  desc: ["Domaine de MIG 1", "Domaine de MIG 2", "Domaine de MIG 3", "Projet professionnel", "Information supplémentaire sur le projet professionnel", "Période privilégiée pour réaliser des missions", "Choix 1 période", "Choix 2 période", "Choix 3 période", "Choix 4 période", "Choix 5 période", "Mobilité aux alentours de son établissement", "Mobilité aux alentours de son domicile", "Mobilité aux alentours d'un de ses proches", "Adresse du proche", "Mode de transport", "Format de mission", "Engagement dans une structure en dehors du SNU", "Description engagement ", "Souhait MIG"],
  fields: ["youngId", "professionnalProjectPrecision", "period"]
}, {
  id: "representative1",
  title: "Représentant légal 1",
  desc: ["Statut représentant légal 1", "Prénom représentant légal 1", "Nom représentant légal 1", "Email représentant légal 1", "Téléphone représentant légal 1", "Adresse représentant légal 1", "Code postal représentant légal 1", "Ville représentant légal 1", "Département représentant légal 1", "Région représentant légal 1"],
  fields: ["youngId"]
}, {
  id: "representative2",
  title: "Représentant légal 2",
  desc: ["Statut représentant légal 2", "Prénom représentant légal 2", "Nom représentant légal 2", "Email représentant légal 2", "Téléphone représentant légal 2", "Adresse représentant légal 2", "Code postal représentant légal 2", "Ville représentant légal 2", "Département représentant légal 2", "Région représentant légal 2"],
  fields: ["youngId"]
}];
...
var youngPlanDeTranportExportFields = [{
  ...
  title: "Représentant légal 1",
  desc: ["Statut", "Nom", "Prénom", "Email", "Téléphone", "Adresse", "Code postal", "Ville", "Département et région du représentant légal"],
  fields: ["parent1Status", "parent1FirstName", "parent1LastName", "parent1Email", "parent1Phone", "parent1PhoneZone", "parent1Address", "parent1Zip", "parent1City", "parent1Department", "parent1Region"]
}, {
  id: "representative2",
  title: "Représentant légal 2",
  desc: ["Statut", "Nom", "Prénom", "Email", "Téléphone", "Adresse", "Code postal", "Ville", "Département et région du représentant légal"],
  fields: ["parent2Status", "parent2FirstName", "parent2LastName", "parent2Email", "parent2Phone", "parent2PhoneZone", "parent2Address", "parent2Zip", "parent2City", "parent2Department", "parent2Region"]
}, {
  id: "phase1Affectation",
  title: "Phase 1 - Affectation ",
  desc: ["ID", "Code", "Nom", "Ville", "Département et région du centre"],
  ...
  id: "phase1Transport",
  title: "Phase 1 - Transport",
  desc: ["ID du transport", "ID du point de rassemblement", "Nom du point de rassemblement", "Adresse du point de rassemblement", "Ville du point de rassemblement", "Département du point de rassemblement", "Région du point de rassemblement", "Date et heure de départ (Aller)", "Date et heure d'arrivée (Retour)"],
...
var missionCandidatureExportFields = [{
  ...
  title: "Candidature",
  desc: ["Statut de la candidature", "Choix - Ordre de la candidature", "Candidature créée le", "Candidature mise à jour le", "Statut du contrat d'engagement", "Pièces jointes à l’engagement", "Statut du dossier d'éligibilité PM"],
  fields: ["youngId", "status", "priority", "createdAt", "updatedAt", "contractAvenantFiles", "justificatifsFiles", "feedBackExperienceFiles", "othersFiles", "missionId", "contractStatus", "eligibilityStatus"]
}, {
  id: "missionInfo",
  title: "Informations sur la mission",
  desc: ["ID de la mission", "Titre de la mission", "Date du début", "Date de fin", "Domaine d'action principal", "Préparation militaire"],
  ...
  title: "Représentant légal 1",
  desc: ["Statut", "Nom", "Prénom", "Email", "Téléphone", "Adresse", "Code postal", "Ville", "Département et région du représentant légal"],
  fields: ["parent1Status", "parent1FirstName", "parent1LastName", "parent1Email", "parent1Phone", "parent1Address", "parent1Zip", "parent1City", "parent1Department", "parent1Region"]
}, {
  id: "representative2",
  title: "Représentant légal 2",
  desc: ["Statut", "Nom", "Prénom", "Email", "Téléphone", "Adresse", "Code postal", "Ville", "Département et région du représentant légal"],
  fields: ["parent2Status", "parent2FirstName", "parent2LastName", "parent2Email", "parent2Phone", "parent2Address", "parent2Zip", "parent2City", "parent2Department", "parent2Region"]
}];
...
var getResultLabel = function getResultLabel(e, pageSize) {
  return "".concat(pageSize * e.currentPage + 1, "-").concat(pageSize * e.currentPage + e.displayedResults, " sur ").concat(e.numberOfResults);
};
var getLabelWithdrawnReason = function getLabelWithdrawnReason(value) {
  var _WITHRAWN_REASONS$fin;
  return ((_WITHRAWN_REASONS$fin = WITHRAWN_REASONS.find(function (e) {
    return e.value === value;
  })) === null || _WITHRAWN_REASONS$fin === void 0 ? void 0 : _WITHRAWN_REASONS$fin.label) || value;
};
