...
export class ExporterMissions implements UseCase<ExporterMissionsResult> {
    ...
    mapMission(mission: MissionType, selectedFields: string[], auteur: ExportMissionsTaskParameters["auteur"]) {
        const allFields = {
            ...
            missionDetails: {
                "Objectifs de la mission": htmlToPlainText(mission.description),
                "Actions concrètes": htmlToPlainText(mission.actions),
                Contraintes: htmlToPlainText(mission.contraintes),
                Durée: mission.duration,
                "Fréquence estimée": htmlToPlainText(mission.frequence),
