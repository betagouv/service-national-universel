...
export class ModifierReferentClasse implements UseCase<ReferentModel> {
    ...
    async execute(classeId: string, modifierReferentClasseModel: ModifierReferentClasseModel): Promise<ReferentModel> {
        ...
        if (currentReferent.email === modifierReferentClasseModel.email) {
        ...
        try {
            ...
            await this.handlePreviousReferent(currentReferent, classe);
    ...
    private async handlePreviousReferent(currentReferent: ReferentModel, classe: ClasseModel) {
        // const currentReferent = await this.referentGateway.findById(classe.referentClasseIds[0]);
        const hasCurrentReferentOtherClasses = await this.classeGateway.findByReferentId(currentReferent.id);
        if (hasCurrentReferentOtherClasses.length === 0) {
            await this.referentService.deleteReferentAndSendEmail(currentReferent);
        } else {
            await this.notificationGateway.sendEmail<SupprimerClasseEngageeParams>(
                {
                    to: [{ email: currentReferent.email, name: `${currentReferent.prenom} ${currentReferent.nom}` }],
