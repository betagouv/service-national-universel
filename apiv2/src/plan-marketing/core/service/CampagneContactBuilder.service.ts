...
export class CampagneContactBuilderService {
    ...
    buildYoungContactRow(
        ...
        return {
            ...
            HEURE_ALLER: segmentDeLignes.find((segment) => segment?.ligneDeBusId === young.ligneId && segment?.pointDeRassemblementId === young.meetingPointId)?.heureDepart,
            DATE_RETOUR: this.clockGateway.isValidDate(dateRetour) ? this.clockGateway.formatShort(dateRetour) : "",
            HEURE_RETOUR: segmentDeLignes.find((segment) => segment?.ligneDeBusId === young.ligneId && segment?.pointDeRassemblementId === young.meetingPointId)?.heureRetour,
            PRENOM_RL1: "",
            NOM_RL1: "",
            PRENOM_RL2: "",
            NOM_RL2: "",
