...
import { updateYoungStayPresenceJDM } from "../../../services/young.service";
...
const ModalChangePresenceJDM = ({ young, value, onConfirm, onLoading, ...rest }) => {
  ...
  const handleConfirm = async () => {
    try {
      setIsLoading(true);
      onLoading(true);
      const { data } = await updateYoungStayPresenceJDM({ youngId: young._id, value });
      onConfirm(data);
    } catch (error) {
...
export default ModalChangePresenceJDM;
