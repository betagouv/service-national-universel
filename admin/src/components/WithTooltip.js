...
export default function WithTooltip({ children, tooltipText, ...props }) {
  ...
  return (
    <div className="relative flex flex-col items-center group" {...props}>
      {children}
      <div className="absolute flex flex-col hidden group-hover:flex bottom-0 mb-3 items-center">
        <span className="relative py-2 px-2.5 text-xs leading-none text-[#414458] whitespace-no-wrap bg-white shadow-sm z-[500] rounded-lg">{tooltipText}</span>
