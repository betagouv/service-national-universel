...
export default function Historic({ model, data, customFilterOptions, refName }) {
  ...
  const pathOptions = getOptions("path")
    .map((e) => ({ label: translateModelFields(model, e), value: e }))
  ...
  function getOptions(key) {
    const arr = data?.map((e) => e[key]).sort();
    const set = [...new Set(arr)];
    console.log("🚀 ~  file: Historic2.js:28 ~ getOptions ~ set", set);
    return set;
  }
...
function Combobox() {
  ...
  return (
    <Combobox as="div" value={selectedPerson} onChange={setSelectedPerson} multiple>
      ...
      <div className="relative mt-1">
        ...
        <Combobox.Button className="absolute inset-y-0 right-0 flex items-center rounded-r-md px-2 focus:outline-none">
          <BsChevronUp className="h-5 w-5 text-gray-400" aria-hidden="true" />
        </Combobox.Button>
...
function MultiSelect({ options, selected, onChange, label }) {
  const [query, setQuery] = useState("");
  console.log("🚀 ~ file: Historic2.js:157 ~ MultiSelect ~ query", query);
          ...
          <div className="relative">
            ...
            <Transition show={open} as={Fragment} leave="transition ease-in duration-100" leaveFrom="opacity-100" leaveTo="opacity-0">
              <Listbox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
                <input onChange={(e) => setQuery(e.target.value)} placeholder="Rechercher" />
