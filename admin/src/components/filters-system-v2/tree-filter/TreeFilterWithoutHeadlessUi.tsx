            ...
            {isOpen && treeFilter["0"] && (
              <div className="absolute left-0 z-10 mt-2 w-64 rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5">
                ...
                <div className="flex flex-col gap-1 py-2">
                  {treeFilter["0"].childIds?.map((childId) => <InnerLevelTreeFilter key={`root-${id}-${childId}`} nodeId={childId} />)}
                </div>
              </div>
            )}
          </div>
...
function onRender(id, phase, actualDuration, baseDuration, startTime, commitTime) {
  console.log(JSON.stringify({ id, phase, actualDuration, baseDuration, startTime, commitTime }));
}
