...
export default function Combobox({ label, setQuery, options = [], value, onChange }) {
  return (
    <HeadlessCombobox as="div" value={value} onChange={onChange}>
      <HeadlessCombobox.Label className="block text-sm font-medium leading-6 text-gray-800">{label}</HeadlessCombobox.Label>
          ...
          <HeadlessCombobox.Options className="absolute z-10 mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
            {options.map((option) => (
              <HeadlessCombobox.Option
                key={option.label}
                value={option}
                className={({ active }) => classNames("relative cursor-default select-none py-2 pl-3 pr-9", active ? "bg-blue-600 text-white" : "text-gray-800")}>
                {({ active, selected }) => (
                  <>
