...
export default function ModalTimeSchedule({ session, onCancel }) {
  return (
      ...
      <ModalContainer className="p-8">
        ...
        <div className="w-full">
          {session.timeScheduleFiles && session.timeScheduleFiles.length > 0 ? (
            session.timeScheduleFiles.map((file) => <TimeScheduleFile session={session} file={file} key={file.name} />)
          ) : (
...
function TimeScheduleFile({ session, file, className = "" }) {
