...
export const CampagneEnvoiStatus: React.FC<CampagneEnvoiStatusProps> = ({ envois, campagneId }) => {
  ...
  return (
    <div className="flex-1">
      <div className="flex-1 flex flex-row gap-4">
        <div>{firstEnvoi && <Badge key={0} title={`Envoyée le ${formatDateFRTimezoneUTC(firstEnvoi.date)}`} status={"VALIDATED"} />}</div>
        <div>
