...
async function transform(data, selectedFields) {
  ...
  return all.map((data) => {
    ...
    const allFields = {
      ...
      missionDetails: {
        "Objectifs de la mission": htmlToPlainText(data.description),
        "Actions concrètes": htmlToPlainText(data.actions),
        Contraintes: htmlToPlainText(data.contraintes),
        Durée: data.duration,
        "Fréquence estimée": htmlToPlainText(data.frequence),
