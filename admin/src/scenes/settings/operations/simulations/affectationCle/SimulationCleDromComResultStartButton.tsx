...
import { AffectationRoutes, formatDepartement, region2department, RegionsDromComEtCorse, SimulationAffectationCLETaskDto, TaskStatus, translate } from "snu-lib";
...
interface SimulationCleDromComResultStartButtonProps {
  simulation: unknown;
}

export default function SimulationCleDromComResultStartButton({ simulation }: SimulationCleDromComResultStartButtonProps) {
  ...
  const affectationKey = ["affectation", "cle-dromcom", simulationCle.metadata!.parameters!.sessionId]; // check AffectationCLESimulationDromComModal.tsx queryKey
  const {
    isPending: isLoading,
    isError,
    ...
    queryFn: async () => AffectationService.getAffectation(simulationCle.metadata!.parameters!.sessionId, "CLE_DROMCOM"),
  });
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: async () => {
      return await AffectationService.postValiderAffectationCleDromCom(simulationCle.metadata!.parameters!.sessionId!, simulationCle.id);
    },
    onSuccess: () => {
