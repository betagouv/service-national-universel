...
interface BasculeJeuneNonValidesStartButtonProps {
  simulation: unknown;
}

export default function BasculeJeuneNonValidesStartButton({ simulation }: BasculeJeuneNonValidesStartButtonProps) {
  ...
  const affectationKey = ["inscription", "bascule-jeunes-non-valides", simulationBascule.metadata!.parameters!.sessionId]; // check BasculeJeuneValidesModal.tsx queryKey
  const {
    isPending: isLoading,
    isError,
    data: affectationStatus,
  } = useQuery<InscriptionRoutes["GetBasculeJeunesNonValides"]["response"]>({
    queryKey: affectationKey,
    queryFn: async () => InscriptionService.getBasculeJeunesNonValides(simulationBascule.metadata!.parameters!.sessionId),
  });
  ...
  const { isPending, mutate } = useMutation({
    mutationFn: async () => {
      return await InscriptionService.postValiderBasculeJeunesNonValides(simulationBascule.metadata!.parameters!.sessionId!, simulationBascule.id, { sendEmail });
    },
    onSuccess: () => {
