...
export default function YoungHeader({ young, tab, onChange, phase = YOUNG_PHASE.INSCRIPTION, isStructure = false, applicationId = null }) {
  ...
  const canYoungChangeCohort = () => {
    if (young.status === YOUNG_STATUS.DELETED || [ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role)) {
      return false;
    }
  ...
  return (
    <div className="flex items-end justify-end border-b-[1px] border-b-[#E5E7EB] px-[30px] pt-[15px]">
      ...
      <div
        className={`flex w-full flex-row flex-wrap-reverse items-end justify-end gap-2 ${
          [ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role) ? "pt-[57px]" : ""
        }`}>
        <div className="grow self-start">
            ...
            <Title>
              <div className="mr-[15px]">
                <div className="flex items-center">
                  {![ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role) && getNotesByPhase("").length > 0 && (
                    <NoteIcon className="mr-1" onClick={setViewedNoteParPhase("")} />
                  )}
          ...
          {isStructure ? (
            <TabList className="mt-[30px]">
              ...
              <Tab isActive={tab === "dossier"} onClick={() => history.push(`/volontaire/${young._id}/phase2/application/${applicationId}/dossier`)}>
                <div className="flex items-center">
                  ...
                  {![ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role) && getNotesByPhase(PHASE_INSCRIPTION).length > 0 && (
            ...
            <TabList className="mt-[30px]">
              ...
              {![ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role) && canViewNotes(user) && (
        ...
        {!isStructure && ![ROLES.HEAD_CENTER, ROLES.HEAD_CENTER_ADJOINT, ROLES.REFERENT_SANITAIRE].includes(user.role) && (
