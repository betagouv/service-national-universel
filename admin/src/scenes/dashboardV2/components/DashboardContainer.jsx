...
const TabItem = ({ id, active, label, route, icon }) => {
  let Icon = icon;
  return (
    <Link className={`cursor-pointer gap-2 h-9 px-2 ${active === id ? "border-b-2  border-blue-600 " : ""}`} to={route}>
      <div className="flex items-center gap-1.5">
        <Icon size={20} className={`${active === id ? "text-blue-600" : "text-gray-500"}`} />
