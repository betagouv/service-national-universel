...
export default function ModalConfirmDelete({ isOpen, onCancel, onConfirm, title, message }) {
  return (
    <ModalTailwind isOpen={isOpen} onClose={onCancel} className="w-[512px] bg-white rounded-lg shadow-xl">
      <div className="flex flex-col p-8 w-full items-center gap-4">
        ...
        <div className="flex items-center gap-4 w-full mt-4">
          ...
          <button onClick={onConfirm} className="border-[1px] border-red-600 text-white bg-red-600 py-2 w-1/2 rounded-lg hover:shadow-ninaButton ">
            Supprimer
          </button>
        </div>
      </div>
