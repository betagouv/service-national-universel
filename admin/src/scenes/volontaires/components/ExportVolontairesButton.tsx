...
import { translate, UserDto, youngExportFields } from "snu-lib";
...
export default function ExportVolontairesButton({ user, selectedFilters, disabled }: Props) {
  ...
  const { mutate: exportJeunes, isPending } = useMutation<any, Error, { selectedFilters: { [key: string]: Filter }; selectedFields: string[] }>({
    ...
    onSuccess: () => {
      toastr.success("Exportation des volontaires", "L'exportation des volontaires a en cours de traitement, vous recevrez un email lorsque cela sera terminé.");
      setExportParams(null);
      setShowModal(null);
    },
    onError: (error) => {
      toastr.error("Une erreur est survenue lors de l'exportation des volontaires", translate(error.message));
    },
  });
