...
import { getFilterLabel, YOUNG_STATUS_PHASE1, translate, getAge, SHOW_ASSIGN_CENTER } from "../../../utils";
                  ...
                  <Table>
                    <thead>
                      <tr>
                        ...
                        {SHOW_ASSIGN_CENTER && ["admin", "referent_region"].includes(user.role) ? <th>Affectation</th> : null}
                      </tr>
                    </thead>
                    <tbody>
                      {list.map((hit, i) => (
...
const Hit = ({ index, hit, onSend, onClick, selected, onChangeYoung }) => {
  ...
  return (
    <tr style={{ backgroundColor: selected && "#e6ebfa" }} onClick={onClick}>
      ...
      {SHOW_ASSIGN_CENTER && ["admin", "referent_region"].includes(user.role) ? (
