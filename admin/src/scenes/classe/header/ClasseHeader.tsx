...
import { translateStatusClasse, ClassesRoutes, canInviteYoung, COHORT_TYPE, TaskStatus } from "snu-lib";
...
export default function ClasseHeader({ classe, setClasse, isLoading, setIsLoading, studentStatus, page }: Props) {
  ...
  const isImportInProgress = inscriptionMasseStatus && [TaskStatus.IN_PROGRESS, TaskStatus.PENDING].includes(inscriptionMasseStatus?.status as TaskStatus);
  ...
  return (
    <>
      {isImportInProgress && <InfoMessage title="Un import de liste d'élèves est en cours de chargement..." icon={HiOutlineRefresh} className="mb-4" />}
      <Header
