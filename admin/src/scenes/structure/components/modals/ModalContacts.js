...
export default function ModalContacts({ isOpen, setIsOpen, contacts, getContacts, createContact, updateContact, deleteContact }) {
  ...
  const handleDelete = async () => {
    setIsLoading(true);
    try {
      await deleteContact(edit.contactId);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const { ok } = await createContact(value);
      if (!ok) {
        resetState();
...
const Contact = ({ contact, setHit }) => {
  console.log("🚀 ~ file: ModalContacts.js:201 ~ Contact ~ contact", contact);
