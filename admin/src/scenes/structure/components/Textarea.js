...
export default function Textarea({ onChange, value, label, disabled = false, error, readOnly = false, copy }) {
  ...
  return (
    <div className={`flex flex-col border-[1px] border-gray-300 w-full py-2 px-2.5 rounded-lg ${disabled ? "bg-gray-100" : ""} ${error ? "border-red-500" : ""}`}>
      ...
      <div className="flex items-center gap-2">
        ...
        <textarea className={`w-full ${disabled ? "bg-gray-100" : ""} ${readOnly && "cursor-default"}`} value={value} onChange={onChange} disabled={disabled} readOnly={readOnly} />
      </div>
