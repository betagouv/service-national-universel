...
export default function MultiSelect({ options, selected, setSelected, label, readOnly = false, icon, error }) {
  return (
    <Listbox value={selected} onChange={setSelected} multiple className="bg-green-500 w-full">
      {({ open }) => (
        <div>
          <Listbox.Button className="relative text-left">
            <div
              className={`${!readOnly ? "cursor-default" : "cursor-pointer"} rounded-lg border ${error ? "border-red-500" : "border-gray-300"} bg-white py-2 px-2.5 items-center`}>
              {icon ? icon : null}
              <label className="text-xs leading-4 text-gray-500">{label}</label>
              <div className="bg-red-200 flex gap-2 items-center justify-between overflow-hidden">
                {selected.map((e) => (
