...
export default function YoungListHeaderValidation({ selectAll, onSelectAllChange, selectedYoungs, onSelectedYoungsChange, youngList }: Props) {
  ...
  return (
    <tr className="flex items-center py-1 px-4 text-xs font-[500] leading-5 uppercase text-gray-500 bg-gray-50 cursor-default">
      <th className="w-[5%]">
        <input className="cursor-pointer w-5 h-5 ml-1 my-2" type="checkbox" checked={selectAll} onChange={handleSelectAll} />
      </th>
      <th className="w-[25%]">Élèves</th>
      <th className="w-[30%]">Classes</th>
      <th className="w-[30%]">Statuts</th>
      <th className="w-[10%]">Inscriptions</th>
    </tr>
  );
}
