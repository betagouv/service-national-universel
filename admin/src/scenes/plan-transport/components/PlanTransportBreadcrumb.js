...
export default function PlanTransportBreadcrumb({ className = "", region = null, department = null }) {
  function goToNational() {
    console.log("go to national");
  }
  ...
  return (
    <div className={`flex ${className}`}>
      <Crumb color="#E5E7EB" onClick={goToNational} className="z-20" key="national">
        <FlagFr />
      </Crumb>
      {region && (
        <Crumb color="#D1D5DB" onClick={goToRegion} className="ml-[-12px] z-10" key="region">
          {region.label}
        </Crumb>
      )}
      {department && (
