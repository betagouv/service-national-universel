...
export default function TimePicker({ value, onChange, disabled }) {
  ...
  const [hour, setHour] = React.useState(time[0]);
  const [min, setMin] = React.useState(time[1]);
  ...
  return (
    <div style={{ fontFamily: "Marianne" }} ref={ref}>
      <div className="relative">
        ...
        {/* display options */}
        <div
          className={`${open ? "flex" : "hidden"} absolute min-w-[100px] rounded-lg bg-white transition left-0 border-3 z-50  h-[200px] border-red-600 shadow`}>
            <div className="block overflow-scroll w-1/2">
                {optionsHour.map((option, index) => (
                    <div key={index} onClick={() => handleChangeHour(option)} className={`${option === hour && "bg-gray font-bold"}`}>
            ...
            <div className="block overflow-scroll w-1/2">
                {optionsMin.map((option, index) => (
                    <div key={index} onClick={() => handleChangeMin(option)} className={`${option === min && "bg-gray font-bold"}`}>
                    <div className="group flex cursor-pointer items-center justify-between gap-2 p-2 px-3 text-gray-700 hover:bg-gray-50">
                        <div dangerouslySetInnerHTML={{ __html: option }} />
