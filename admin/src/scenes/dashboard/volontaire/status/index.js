...
export default ({ filter }) => {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (filter.cohort?.length) body.query.bool.filter.push({ term: { "cohort.keyword": filter.cohort } });
    ...
    (async () => {
      ...
      if (filter.cohort?.length) body.query.bool.filter.push({ term: { "youngCohort.keyword": filter.cohort } });
