...
command=$(env | grep DOCKER_ENV_ | sed 's/^\(.*\)=\(.*\)$/-e ###s|\1|\2|g###/g' | xargs echo sed -i.bak | sed 's/###/\"/g')
# Apply the substitution only on the index-*.js file
eval $command $(find /service-national-universel/admin/build/assets -name "index-*.js" -type f | head -n 1)
