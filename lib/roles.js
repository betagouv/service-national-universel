...
function canUpdateReferent({ actor, originalTarget, modifiedTarget = null, structure }) {
  ...
  const isAdmin = actor.role === ROLES.ADMIN;
  const withoutChangingRole = (modifiedTarget === null || modifiedTarget.role === originalTarget.role);
  ...
  const isMeWithoutChangingRole =
    ...
    (modifiedTarget === null || !('role' in modifiedTarget) || modifiedTarget.role === originalTarget.role);
