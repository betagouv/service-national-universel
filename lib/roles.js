...
function canDeleteReferent({ actor, originalTarget, structure }) {
  ...
  if (actor._id === originalTarget._id) return false;
...
<<<<<<< Updated upstream
=======
  // Responsible from same structure
  const responsibleFromSameStructure =
    actor.role === ROLES.RESPONSIBLE &&
    actor.structureId === originalTarget.structureId;
  // const supervisorFromStructure =
  //   actor.role === ROLES.SUPERVISOR &&
  //   actor.supervisorFromStructure(originalTarget.structureId);
...
=======
    headCenterFromTheSameDepartment ||
    // supervisorFromStructure ||
    responsibleFromSameStructure;
>>>>>>> Stashed changes
  return authorized;
}
