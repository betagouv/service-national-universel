...
function canCreateOrUpdateSessionPhase1(actor, target) {
  ...
  const isHeadCenter =
    actor.role === ROLES.HEAD_CENTER && target && actor._id.toString() === target.headCenterId;
...
function canCreateOrUpdateStructure(actor) {
  return [ROLES.ADMIN, ROLES.SUPERVISOR].includes(actor.role);
}
