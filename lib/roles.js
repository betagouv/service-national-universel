...
const ROLES_READABLE = {
  admin: "Admin",
  referent_department: "Référent Département",
  referent_region: "Référent Région",
...
const SUB_ROLES_READABLE = {
  manager_department: "Manager Département",
  assistant_manager_department: "Assistant Manager Département",
  manager_phase2: "Manager Phase 2",
  secretariat: "Secrétariat",
  coordinator: "Coordinateur",
  assistant_coordinator: "Assistant Coordinateur",
  none: "",
};
...
const SUPPORT_ROLES_LIST = Object.values(SUPPORT_ROLES).filter(Boolean);
...
function canUpdateReferent(actor, originalTarget, modifiedTarget) {
  ...
  const isReferentModifyingReferentWithoutChangingRole =
    ...
    // ... modifying referent ...
    [ROLES.REFERENT_DEPARTMENT, ROLES.REFERENT_REGION].includes(originalTarget.role) &&
    // ... witout changing its role.
    [ROLES.REFERENT_DEPARTMENT, ROLES.REFERENT_REGION].includes(modifiedTarget.role);
  const authorized =
    isAdmin ||
