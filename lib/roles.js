...
function canDeleteReferent({ actor, originalTarget, structure }) {
  ...
  const isResponsable = actor.role === ROLES.SUPERVISOR;
  ...
  const responsibleFromSameStructure =
    targetIsResponsible && actor.structureId === originalTarget.structureId;
  const supervisorOfMainStructure =
    isSupervisor && actor.structureId === (structure && structure.networkId);
