...
const ROLES_TO_FIELDS = {
  head_center: ["sessionPhase1Id", "cohorts"],
...
const canDeleteYoung = (actor, target) => isAdmin(actor) || referentInSameGeography(actor, target);
...
function canUpdateReferent({ actor, originalTarget, modifiedTarget = null, structure }) {
  ...
  const geographicTargetData = {
    region: originalTarget.region || structure?.region,
    department: originalTarget.department?.length ? originalTarget.department : [structure?.department],
  };
