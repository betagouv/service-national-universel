...
function canUpdateReferent({
  ...
  const isReferentModifyingReferentWithoutChangingRole =
    ...
    // if the actor is a REFERENT_REGION check if the actor and the target in the region
    (actor.role !== ROLES.REFERENT_REGION ||
      actor.region === (originalTarget.region || structure?.region)) &&
    // if the actor is a REFERENT_DEPARTMENT check if the actor and the target in the department
