...
function inscriptionCreationOpenForYoungs(cohort, allowed) {
...
const SESSIONPHASE1ID_CANCHANGESESSION = ["627cd8b873254d073af93147"];

const youngCanChangeSession = ({ cohort, statusPhase1, status, sessionPhase1Id }) => {
  if (status === YOUNG_STATUS.WAITING_LIST) return true;
  if (statusPhase1 === YOUNG_STATUS_PHASE1.AFFECTED && SESSIONPHASE1ID_CANCHANGESESSION.includes(sessionPhase1Id)) {
    return true;
  }
  if (
    [
