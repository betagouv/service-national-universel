...
const youngCanChangeSession = ({ cohort, statusPhase1, status, sessionPhase1Id }) => {
  if ([YOUNG_STATUS.REFUSED, YOUNG_STATUS.WITHDRAWN].includes(status)) return false;
  if ([YOUNG_STATUS.WAITING_LIST, YOUNG_STATUS.WAITING_VALIDATION, YOUNG_STATUS.WAITING_CORRECTION].includes(status)) return true;
  if ([YOUNG_STATUS_PHASE1.AFFECTED].includes(statusPhase1) && status === YOUNG_STATUS.VALIDATED) {
    return true;
  }
  ...
  if ([YOUNG_STATUS_PHASE1.AFFECTED, YOUNG_STATUS_PHASE1.WAITING_AFFECTATION].includes(statusPhase1)) {
    const now = Date.now();
  ...
  if (YOUNG_STATUS_PHASE1.NOT_DONE) return true;
  return false;
};
