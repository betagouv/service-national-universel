...
const TicketsPreview = ({ activeTicketsIds, handleRemoveTicketFromPreview }) => {
  ...
  return (
    <ResizablePanel className={`grow-0 shrink-0 border-l-2 z-10 overflow-hidden flex flex-col h-80 w-full relative`} position="bottom" name="admin-tickets-bottom-panel">
      <div className="relative shrink-0 flex flex-col h-full overflow-x-hidden">
        ...
        <div className="w-full flex flex-shrink-1 gap-3 px-2 max-w-full overflow-hidden justify-end">
          {tickets.map((ticket) => (
            <button
              ...
              style={{ flexBasis: "25%" }}
              className={`w-16 flex justify-between py-2 px-4 grow-0 flex-shrink bg-snu-purple-100 hover:bg-snu-purple-300 rounded-t-lg rounded-b-none border-none text-snu-purple-900 hover:text-white ${
                activeTicketId === ticket._id ? "!text-white !bg-snu-purple-900" : ""
              }`}
            >
              <span className="whitespace-nowrap truncate grow-0">{ticket.title}</span>
              <svg
                data-ticketid={ticket._id}
