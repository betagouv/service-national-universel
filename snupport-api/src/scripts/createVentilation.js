...
async function createRegionVentilation() {
  try {
    let i = 0;
    for (let department of Object.values(departmentLookUp)) {
      let folder = await FolderModel.findOne({ name: department });
      let ventilation = {
        name: "Ventilation référent régional dossier : " + department + " région : " + department2region[department],
        description: "Ventilation référent régional dossier : " + department,
        active: true,
        userRegion: department2region[department],
        userRole: "REFERENT_REGION",
        conditionsEt: [{ field: "contactDepartment", operator: "is", value: department }],
        actions: [{ action: "SET", field: "foldersId", value: folder._id }],
      };
      await VentilationModel.create(ventilation);
      console.log(i);
      i++;
    }
...
/* eslint-disable no-unused-vars */
async function createPhaseVentilationRegion() {
  try {
    let i = 0;
    for (let region of regionList) {
      let folder = await FolderModel.findOne({ name: "Phase 3", userRole: "REFERENT_REGION", userRegion: region });
      let ventilation = {
        name: "Ventilation référent régional dossier : Phase 3 Région : " + region,
        description: "Ventilation référent régional ",
        active: true,
        userRegion: region,
        userRole: "REFERENT_REGION",
        conditionsEt: [{ field: "formSubjectStep2", operator: "is", value: "PHASE_3" }],
        actions: [{ action: "SET", field: "foldersId", value: folder._id }],
      };
      await VentilationModel.create(ventilation);
      console.log(folder);
...
/* eslint-disable no-unused-vars */
async function createPhaseVentilationDepartment() {
  try {
    let i = 0;
    for (let department of Object.values(departmentLookUp)) {
      let folder = await FolderModel.findOne({ name: "Phase 0 - Inscription", userRole: "REFERENT_DEPARTMENT", userDepartment: department });
      let ventilation = {
        name: "Ventilation référent départemental dossier : Phase 0 - Inscription Département : " + department,
        description: "Ventilation référent départemental ",
        active: true,
        userDepartment: department,
        userRole: "REFERENT_DEPARTMENT",
        conditionsEt: [{ field: "formSubjectStep2", operator: "is", value: "PHASE_0" }],
        actions: [{ action: "SET", field: "foldersId", value: folder._id }],
      };
      await VentilationModel.create(ventilation);
      console.log(folder);
