...
router.post("/application-change-status-canceled-validated/count", async (req, res) => {
  try {
    const { error, value } = Joi.object({
    ...
    if (value.structureId) {
      result = await db.query(
        `
        select count (distinct candidature_id) from "public"."logs_by_day_application_status_change_event"
    ...
    capture(error);
    res.status(500).send({ ok: false, code: "Error in application-change-status-canceled-validated" });
  }
});
