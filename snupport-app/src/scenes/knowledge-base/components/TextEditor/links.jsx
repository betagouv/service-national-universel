...
export const AddLinkModal = ({ isOpen, onRequestClose }) => {
  ...
  const computeError = () => {
    const isSlug = flattenedKB.map((item) => `/base-de-connaissance/${item.slug}`).includes(url);
    if (url.startsWith("/")) {
  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={onRequestClose}>
      <form onSubmit={onSubmit} className="w-screen-3/4 flex flex-col items-start overflow-hidden">
