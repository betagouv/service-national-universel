  ...
  return (
    <Modal isOpen={isOpen} onRequestClose={onRequestClose}>
      <div className="overflow-hidden">
        <div className="w-screen-3/4 flex h-full flex-shrink  flex-col overflow-hidden">
...
export const BlueIcon = ({ icon, className }) => (
  <div key={icon} className={`my-5 mx-10 inline-flex w-12 flex-col items-center justify-center ${className || ""}`}>
    <span className={`material-icons mb-2 flex h-10 w-10 items-center justify-center rounded-md bg-[#32257F] text-center text-2xl text-white`}>{icon}</span>
  </div>
);
