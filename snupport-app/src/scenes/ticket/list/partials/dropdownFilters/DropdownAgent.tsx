...
export default function DropdownAgent({ name, selectedAgent, setSelectedAgent, agents }) {
      ...
      <Popover className="relative grow">
        ...
        >
          <Popover.Panel className="absolute z-10 mt-2 flex w-full min-w-[224px] origin-top-left flex-col rounded-md bg-white py-1 shadow-lg ring-1 ring-black/5">
            {agents?.map((agent) => (
              <Checkbox key={agent._id} name={`${agent.firstName} ${agent.lastName}`} state={selectedAgent.includes(agent._id)} setState={(v) => handleChangeState(agent._id, v)} />
            ))}
          </Popover.Panel>
        </Transition>
      </Popover>
